...
\addbibresource{TP_Presentation.bib}
...
\pause{}
\begin{itemize}
    \item We can refine the function witnessing $\mathsf{\Pi^M_2} \leq^M \mathsf{Fg}$ by iteratively pruning the $\mathbb{P}(\mathcal{T})$s.
    \pause{}
    \item We can also prove in $V$ a few results linking $1$-generic reals with models of certain countable $\Pi_2$ TCIs.
\end{itemize}
\end{frame}
